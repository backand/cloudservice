    ...
    link: function ($scope, $el, $attr, $ngModel) {
      ...
      $scope.$watch('ngModel', function (newValue) {
        if (angular.isDefined(newValue) && newValue) {
          if(!angular.isDefined($scope.myFrequency) || (angular.isDefined($scope.myFrequency) && $scope.myFrequency.base != 6)){
            modelChanged = true;
      ...
      $scope.everyMinuteValues = fillEveryMinuteValues();
      ...
      function fillEveryMinuteValues() {
        var values = [];
        for(var i = 1; i < 60; i++) {
          values.push(i);
        }
      ...
      function setInitialValuesForBase(freq) {
        ...
        if (freq.base >= 1) {
          freq.everyMinuteValues = $scope.everyMinuteValues[0];
        }
...
angular.module('angular-cron-jobs').factory('cronService', function () {
  ...
  service.setCron = function (n) {
    ...
    if (n && n.base && n.base == 1) {
      cron[0] = typeof n.everyMinuteValues !== 'undefined' ? '*/' + n.everyMinuteValues : '*';
    }
  ...
  service.fromCron = function (value, allowMultiple) {
    ...
    if (cron[0].startsWith("*/")) {
      frequency.base = 1; // every X minutes
    } else if (cron[0] === '*' && cron[1] === '*' && cron[2] === '*' && cron[3] === '*' && cron[4] === '*') {
